...
export default function TrackOrderPage() {
  ...
  const handleSearch = async () => {
    ...
    try {
      // Option 1: Use dataStore (legacy)
      // const allOrders = dataStore.getOrders() // Note: getOrders, not getAllOrders
      ...
      const orderNum = orderNumber.trim().toLowerCase()
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-md p-4">
                {foundOrder.status === 'pending' && (
                  <p className="text-blue-800">
                    🎯 Your order #{(foundOrder.orderNumber )?.substring(0, 4)} has been received and is being prepared.
                  </p>
                )}
                {foundOrder.status === 'preparing' && (
                  <p className="text-orange-800">
                    👨‍🍳 Your order #{(foundOrder.orderNumber )?.substring(0, 4)}is currently being prepared. Almost ready!
                  </p>
                )}
