...
export const dataStore = {
  ...
  cancelOrder: (orderId: string) => {
    dataStore.updateOrderStatus(orderId, "Cancelled")
  },
  ...
  // Get all orders (for admin table)
  getAllOrders: (): Order[] => {
    return [...orders]
  },

  // Simulate sending a notification when an order is ready [^1]
  sendOrderReadyNotification: async (orderNumber: string, customerName: string, contactNumber: string) => {
    console.log(`Simulating notification for Order ${orderNumber} to ${customerName} (${contactNumber})...`)
    // In a real app, you'd call an SMS/Email API here.
    // Example: await fetch('/api/send-sms', { method: 'POST', body: JSON.stringify({ orderNumber, contactNumber }) });
    await new Promise((resolve) => setTimeout(resolve, 500)) // Simulate network delay
    console.log(`Notification for Order ${orderNumber} simulated successfully!`)
  },
}
